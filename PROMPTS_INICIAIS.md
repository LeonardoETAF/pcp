...
### Prompt 1.8 — Conector de acesso direto ao One (read-only, polling) — ETL
> **Pré-requisitos (externos) — NÃO iniciar sem todos:** (1) usuário read-only no One; (2)
> conectividade VPS↔LAN definida (VPN ou conector on-premise — ver `docs/integracao/acesso-direto-one.md`);
> (3) índices confirmados nas colunas de data (`PEDV_DATC`, `FAT_DTEMI`…); (4) código do cabeçalho
> de produção. **Mapeamento funcional 100% fechado** (ver `docs/integracao/mapeamento-one-para-pcp.md`).
> O One é **PostgreSQL 9.5** → **sem replicação**; é acesso direto por consulta.
```
No pcp-etl, implemente uma fonte de dados ASSÍNCRONA atrás do trait FonteDados (CLAUDE.md
§1/§8) — sem remover o ImportadorArquivo, que segue para backfill. Crie FonteConsultaOne, que
CONSULTA o One (SQLx read-only, TLS, credenciais em env — §7) de forma INCREMENTAL e aplica a
CAMADA ANTICORRUPÇÃO transformando para o contrato (NovaVendaDia/NovoEstoqueSnapshot, doc 05 §2)
com nomes honestos e tipos corretos. Aterrisse o cru num schema "bronze" e transforme para o
domínio (mapeamento em docs/integracao/mapeamento-one-para-pcp.md):
- ESTOQUE (F03005, saldo GLOBAL) — FULL REFRESH por ciclo: codigo_estoque←EST_ITM (join F03001:
  sku←ITM_SKU, produto←ITM_DESC), configuracao←EST_DCONF, qtd_disponivel←EST_QTDD (usar SEMPRE),
  qtd_estoque←EST_QTDE, qtd_reserva←EST_QTDR, fora_de_linha←EST_FLIN; filtrar
...
- VENDAS (pedidos não cancelados) — INCREMENTAL por PEDV_DATC + janela deslizante p/ pegar
  cancelamentos: codigo_estoque←F05001.ITMP_PRD, qtd_vendida←ITMP_QNT EXCLUINDO cancelados
  (ITMP_STPD <> 'CANCELADO'; PEDV_DCAN nulo), dt_ref←F05002.PEDV_DATC, configuracao←ITMP_CONF;
  somar qtd por dia×produto.
- VENDA FATURADA (F10901+F10911) e PRODUÇÃO (F06002/F060015/F06018) como fontes complementares.
Polling configurável (tabelas quentes a cada 1–5 min; catálogo/backfill em baixa carga). Ligue o
gatilho ao canal LISTEN/NOTIFY+SSE já existente (atualiza ao chegar dado novo). Preserve a
idempotência por data. Testes da ACL com bronze sintético, validando 6797/10001/10473 e a
distribuição do doc 08 §3.
```
